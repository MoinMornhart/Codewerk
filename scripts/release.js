...
function changelogSection(file, version) {
  ...
  const start = text.indexOf(`## [Codewerk ${version}]`);
  if (start === -1) return '';
...
const notesFile = path.join(os.tmpdir(), `codewerk-notes-${version}.md`);
fs.writeFileSync(notesFile, notes);
try {
  gh(['release', 'create', tag, '--draft', '--verify-tag', '--title', `Codewerk ${version}`, '--notes-file', notesFile], env);
} finally {
  fs.rmSync(notesFile, { force: true });
}

console.log(`→ Baue Codewerk ${version} und lade den Installer hoch …`);
